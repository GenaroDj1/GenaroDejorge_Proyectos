...
app.route('/api/movies/<int:movie_id>', methods=['GET'])(get_movie) #aca asociamos cada def que creamos con el sql
app.route('/api/movies/<int:movie_id>', methods=['PUT'])(update_movie)
app.route('/api/movies/<int:movie_id>', methods=['DELETE'])(delete_movie)
#int indica que el movie_id sea entero

if __name__ == '__main__': #con esto corre la aplicacion.
    app.run(debug=True)